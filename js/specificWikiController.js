        ...
        if(event.clickNode){
            ...
            if(event.clickNode.image){
                $http.jsonp("http://"+ $scope.ourWiki + ".wikia.com/api.php?action=parse&section=0&page="+event.clickNode.id+"&format=json&callback=JSON_CALLBACK&redirects&prop=text").then(function(data){
                    myEl.html("<div class = 'wiki-title' id='title-container'><h1>" + event.clickNode.id + "</h1></div><div class = 'wiki-content' id='content-container'><img src = '" + event.clickNode.image + "'><p>" + data.parse.text['*'] + "</p></div>");
                });
            } else{
